...
def build_analysis_ready_player_season(
    ...
    if "games_played" in player_season.columns:
        has_games = player_season["games_played"].notna() & (
            player_season["games_played"] > 0
        )
        player_season.loc[has_games, "VAR_per_game"] = (
            player_season.loc[has_games, "VAR"]
            / player_season.loc[has_games, "games_played"]
        )
    ...
    if trades_df is not None and not trades_df.empty:
        ...
        if not trade_acquisitions.empty:
            ...
            trade_info = (
                trade_acquisitions.groupby(["season_year", "player_id"])
                .agg({"trade_week": "first"})
                .reset_index()
            )
            ...
            player_season["acquired_via_trade_flag"] = player_season[
                "trade_week"
            ].notna()
    ...
    output_cols = [
        ...
        "keeper_cost",
        "keeper_surplus",
        "acquired_via_trade_flag",
        "trade_week",
        "champion_team_flag",
    ]

    result = player_season[
        [c for c in output_cols if c in player_season.columns]
    ].copy()
...
def build_manager_season_value(
    ...
    if not teams_df.empty and "manager" in teams_df.columns:
        ...
        if "manager_id" in teams_df.columns:
            # Merge manager_id
            mgr_ids = teams_df[
                ["season_year", "manager", "manager_id"]
            ].drop_duplicates()
            manager_season_keys = manager_season_keys.merge(
                mgr_ids, on=["season_year", "manager"], how="left"
            )
        else:
    ...
    if (
        manager_season_keys.empty
        and not analysis_df.empty
        and "manager" in analysis_df.columns
    ):
    ...
    if manager_season_keys.empty:
        logger.warning(
            "No manager-season combinations found in teams_df or analysis_df"
        )
        return pd.DataFrame()
    ...
    for idx, ms_key in manager_season_keys.iterrows():
        try:
            ...
            logger.warning(
                f"Missing column in manager_season_keys at row {idx}: {e}. Columns: {manager_season_keys.columns.tolist()}"
            )
            continue

        # Get season metadata
        meta = league_meta.get(int(season), {}) if league_meta else {}
        auction_budget = meta.get("auction_budget", config.AUCTION_BUDGET)
        num_teams = meta.get("num_teams", config.NUM_TEAMS)
        total_league_budget = num_teams * auction_budget

        # Get manager's team info
        if not teams_df.empty and "manager" in teams_df.columns:
            manager_teams = teams_df[
                (teams_df["season_year"] == season) & (teams_df["manager"] == manager)
            ]
        else:
        ...
        champion_flag = (
            (manager_standings["final_rank"] == 1).any()
            if not manager_standings.empty
            else False
        )

        # Get draft spending and VAR
        manager_drafts = analysis_df[
            (analysis_df["season_year"] == season) & (analysis_df["manager"] == manager)
        ].copy()

        # Calculate spending
        draft_spend = manager_drafts["cost"].sum()
        keeper_spend = manager_drafts[manager_drafts["is_keeper"] == True]["cost"].sum()
        auction_spend = manager_drafts[manager_drafts["is_keeper"] == False][
            "cost"
        ].sum()
        ...
        draft_var = manager_drafts[
            (manager_drafts["is_keeper"] == False) & (manager_drafts["VAR"].notna())
        ]["VAR"].sum()

        # Keeper VAR
        keeper_var = manager_drafts[
            (manager_drafts["is_keeper"] == True) & (manager_drafts["VAR"].notna())
        ]["VAR"].sum()
        ...
        if waiver_pickups_df is not None and not waiver_pickups_df.empty:
            ...
            if (
                not manager_waivers.empty
                and "var_after_pickup" in manager_waivers.columns
            ):
        ...
        if lifecycle_df is not None and not lifecycle_df.empty:
            ...
            if not manager_lifecycle.empty:
                var_col = (
                    "VAR_total" if "VAR_total" in manager_lifecycle.columns else "VAR"
                )
        ...
        keeper_spending_pct = (
            (keeper_spend / total_spend * 100) if total_spend > 0 else 0
        )
