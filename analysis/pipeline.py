...
from .keepers import analyze_keeper_value, calculate_keeper_surplus
...
from .tiers import assign_actual_tiers, assign_draft_tiers, calculate_tier_hit_rates
from .trades import analyze_trade_impact
from .value_analysis import (
    build_analysis_ready_player_season,
    build_manager_season_value,
)
...
def run_analysis(
    ...
    try:
        ...
        if teams_path.exists():
            ...
            teams_df = teams_df[
                (teams_df["season_year"] >= start_year)
                & (teams_df["season_year"] <= end_year)
            ]
        ...
        if standings_path.exists():
            ...
            standings_df = standings_df[
                (standings_df["season_year"] >= start_year)
                & (standings_df["season_year"] <= end_year)
            ]
    ...
    if champion_blueprint:
        if (
            "blueprint" in champion_blueprint
            and not champion_blueprint["blueprint"].empty
        ):
        ...
        if (
            "comparison" in champion_blueprint
            and not champion_blueprint["comparison"].empty
        ):
    ...
    try:
        weekly_matchups_df = load_weekly_matchups_from_json(
            loader.league_data_dir, start_year, end_year
        )
    ...
    if not weekly_lineups_df.empty:
        try:
            ...
            manager_season_lineup_stats_df = build_manager_season_lineup_stats(
                team_week_perf_df, teams_df if not teams_df.empty else pd.DataFrame()
            )
    ...
    if not weekly_matchups_df.empty:
        try:
            ...
            if not weekly_matchups_df.empty:
                schedule_difficulty_df = calculate_schedule_difficulty(
                    weekly_matchups_df
                )
            else:
                schedule_difficulty_df = pd.DataFrame()
    ...
    if not manager_season_value_df.empty:
        try:
            distribution_df = calculate_manager_outcome_distributions(
                manager_season_value_df
            )
            ...
            signal_strength_df = calculate_manager_signal_strength(
                manager_season_value_df
            )
            rolling_consistency_df = calculate_rolling_consistency(
                manager_season_value_df
            )
        except Exception as e:
    ...
    # Generate plots
    from .plots import (
        plot_champion_vs_field_shares,
        plot_championship_luck_quadrant,
        plot_championships_vs_median_wins,
        plot_mean_vs_std_wins,
        plot_pa_diff_by_manager,
        plot_pf_vs_pa_scatter,
        plot_price_vs_var_by_position,
        plot_var_distribution_by_manager,
        plot_var_per_dollar_by_manager,
        plot_wins_distribution_by_manager,
        plot_wins_vs_expected_wins,
    )
    ...
    if champion_blueprint and not manager_season_value_df.empty:
        plot_champion_vs_field_shares(
            champion_blueprint, manager_season_value_df, output_path
        )
    ...
    if not waiver_pickups_df.empty:
        ...
        if "pickup_type" in waiver_pickups_df.columns:
            archetypes = (
                waiver_pickups_df.groupby(["pickup_type", "position"])
                .agg(
                    {
                        "player_id": "count",
...
def print_console_summary(
    manager_season_value_df: pd.DataFrame, analysis_df: pd.DataFrame
):
    ...
    if not manager_season_value_df.empty:
        ...
        print("-" * 80)
        manager_careers = (
            manager_season_value_df.groupby("manager")
            .agg({"total_VAR": "sum", "total_spend": "sum", "VAR_per_dollar": "mean"})
            .reset_index()
        )
        manager_careers["VAR_per_dollar"] = (
            manager_careers["total_VAR"] / manager_careers["total_spend"]
        )
        ...
        for i, (_, row) in enumerate(manager_careers.head(5).iterrows(), 1):
            print(
                f"{i}. {row['manager']:20s} ${row['VAR_per_dollar']:.3f} VAR/$  (Total VAR: {row['total_VAR']:.1f}, Spend: ${row['total_spend']:.0f})"
            )

    # Top 3 league inefficiencies
    if analysis_df is not None:
        has_var = (
            analysis_df["VAR"].notna()
            & analysis_df["normalized_price"].notna()
            & (analysis_df["normalized_price"] > 0)
        )
        if has_var.any():
            df_with_var = analysis_df[has_var].copy()
            df_with_var["VAR_per_dollar"] = (
                df_with_var["VAR"] / df_with_var["normalized_price"]
            )

            pos_efficiency = (
                df_with_var.groupby("position")
                .agg(
                    {
                        "VAR_per_dollar": "mean",
                        "dollar_per_VAR": lambda x: (
                            df_with_var.loc[x.index, "normalized_price"]
                            / df_with_var.loc[x.index, "VAR"]
                        ).mean(),
                    }
                )
                .reset_index()
            )
            pos_efficiency.columns = [
                "position",
                "avg_VAR_per_dollar",
                "avg_dollar_per_VAR",
            ]
            ...
            print("-" * 80)
            pos_efficiency = pos_efficiency.sort_values(
                "avg_dollar_per_VAR", ascending=False
            )
            for i, (_, row) in enumerate(pos_efficiency.head(3).iterrows(), 1):
                print(
                    f"{i}. {row['position']:10s} ${row['avg_dollar_per_VAR']:.2f} per VAR  ({row['avg_VAR_per_dollar']:.3f} VAR/$)"
                )
...
def generate_summary_report(
    ...
    lines.append(
        f"- Players with VAR: {players_with_var:,} ({100 * players_with_var / total_players:.1f}%)"
    )
    lines.append(
        f"- Players with Points: {players_with_points:,} ({100 * players_with_points / total_players:.1f}%)"
    )
    lines.append("")
    ...
    if has_data.any():
        ...
        best_value = df_with_data.nlargest(10, "VAR_per_dollar")[
            ["player_name", "position", "normalized_price", "VAR", "VAR_per_dollar"]
        ]
        ...
        for _, row in best_value.iterrows():
            lines.append(
                f"| {row['player_name']} | {row['position']} | ${row['normalized_price']:.1f} | {row['VAR']:.1f} | {row['VAR_per_dollar']:.2f} |"
            )
        lines.append("")
        ...
        if not tier_summary.empty:
            ...
            for position in ["QB", "RB", "WR", "TE"]:
                ...
                if not pos_tiers.empty:
                    ...
                    lines.append(
                        f"- **{position}**: {avg_hit_rate:.1%} average hit rate"
                    )
            lines.append("")
        ...
        if not keeper_summary.empty:
            ...
            if total_keepers > 0:
                ...
                if "surplus_VAR_correlation" in keeper_summary.columns:
                    corr = (
                        keeper_summary["surplus_VAR_correlation"].iloc[0]
                        if len(keeper_summary) > 0
                        else None
                    )
                    if pd.notna(corr):
        ...
        if waiver_pickups_df is not None and not waiver_pickups_df.empty:
            ...
            league_winners = len(
                waiver_pickups_df[waiver_pickups_df["pickup_type"] == "LEAGUE_WINNER"]
            )
            solid_starters = len(
                waiver_pickups_df[waiver_pickups_df["pickup_type"] == "SOLID_STARTER"]
            )
            streamers = len(
                waiver_pickups_df[waiver_pickups_df["pickup_type"] == "STREAMER"]
            )
            became_keepers = waiver_pickups_df["became_keeper"].sum()

            lines.append(f"- Total Waiver/FA Pickups: {total_pickups}")
            lines.append(
                f"- League Winners: {league_winners} ({100 * league_winners / total_pickups:.1f}%)"
            )
            lines.append(
                f"- Solid Starters: {solid_starters} ({100 * solid_starters / total_pickups:.1f}%)"
            )
            lines.append(
                f"- Streamers: {streamers} ({100 * streamers / total_pickups:.1f}%)"
            )
            lines.append(
                f"- Became Keepers: {became_keepers} ({100 * became_keepers / total_pickups:.1f}%)"
            )
            lines.append("")
        ...
        if manager_profiles_df is not None and not manager_profiles_df.empty:
            ...
            if "manager_archetype" in manager_profiles_df.columns:
                archetype_counts = manager_profiles_df[
                    "manager_archetype"
                ].value_counts()
