...
def analyze_trade_impact(
    ...
    if "transaction_type" in transactions_df.columns:
        trades = transactions_df[
            transactions_df["transaction_type"].str.contains(
                "trade", case=False, na=False
            )
        ].copy()
    ...
    for trade_id in trades["transaction_id"].unique():
        ...
        trade_week = (
            trade_txns["acquisition_week"].iloc[0]
            if "acquisition_week" in trade_txns.columns
            else None
        )
        ...
        for player_id in team_a_players:
            if lifecycle_df is not None and not lifecycle_df.empty:
                ...
                if not player_results.empty:
                    var_col = (
                        "VAR_total" if "VAR_total" in player_results.columns else "VAR"
                    )
        ...
        team_a_result = (
            "WIN"
            if team_a_net > team_b_net
            else ("LOSS" if team_a_net < team_b_net else "NEUTRAL")
        )
        team_b_result = (
            "WIN"
            if team_b_net > team_a_net
            else ("LOSS" if team_b_net < team_a_net else "NEUTRAL")
        )
