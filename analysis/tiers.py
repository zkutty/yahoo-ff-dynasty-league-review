...
def assign_draft_tiers(df: pd.DataFrame, league_meta: Dict) -> pd.DataFrame:
    ...
    for season in df["season_year"].unique():
        ...
        for position in ["QB", "RB", "WR", "TE"]:
            ...
            # Assign tiers
            pos_drafts["expected_tier"] = (
                pos_drafts["price_rank"] - 1
            ) // tier_size + 1
            ...
            df.loc[pos_mask, "price_rank_within_position"] = pos_drafts[
...
def assign_actual_tiers(df: pd.DataFrame, league_meta: Dict) -> pd.DataFrame:
    ...
    for season in df["season_year"].unique():
        ...
        for position in ["QB", "RB", "WR", "TE"]:
            ...
            pos_results = pos_results.sort_values(
                "fantasy_points_total", ascending=False
            )
            ...
            # Assign tiers
            pos_results["actual_finish_tier"] = (
                pos_results["points_rank"] - 1
            ) // tier_size + 1
            pos_results["points_rank_within_position"] = pos_results["points_rank"]

            # Update original dataframe
            df.loc[pos_mask, "actual_finish_tier"] = pos_results[
                "actual_finish_tier"
            ].values
            df.loc[pos_mask, "points_rank_within_position"] = pos_results[
                "points_rank_within_position"
            ].values

    logger.info(
        f"Assigned actual tiers to {df['actual_finish_tier'].notna().sum()} players"
    )
    return df


def calculate_tier_hit_rates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_with_tiers["hit"] = (
        df_with_tiers["actual_finish_tier"] <= df_with_tiers["expected_tier"]
    )
    ...
    tier_summary = (
        df_with_tiers.groupby(["position", "expected_tier"])
        .agg(
            {
                "player_id": "count",
                "hit": "mean",
