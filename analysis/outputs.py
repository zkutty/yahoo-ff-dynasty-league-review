...
def save_analysis_ready_data(df: pd.DataFrame, output_dir: Path, season: int):
...
def save_tier_summary(tier_summary: pd.DataFrame, output_dir: Path):
...
def save_position_efficiency(df: pd.DataFrame, output_dir: Path):
    ...
    efficiency = (
        df_with_data.groupby(["position", "expected_tier"])
        .agg(
            {
                "player_id": "count",
    ...
    efficiency.columns = [
        ...
        "count",
        "avg_VAR_per_dollar",
        "median_VAR_per_dollar",
        "avg_dollar_per_VAR",
        "median_dollar_per_VAR",
        "avg_price",
        "avg_VAR",
    ]
...
def save_keeper_surplus_summary(keeper_summary: pd.DataFrame, output_dir: Path):
...
def plot_price_vs_var(df: pd.DataFrame, output_dir: Path):
    ...
    for idx, position in enumerate(positions):
        ...
        # Scatter plot
        scatter = ax.scatter(
            pos_data["normalized_price"], pos_data["VAR"], alpha=0.6, s=50
        )
        ...
        try:
            ...
            p = np.poly1d(z)
            x_line = np.linspace(
                pos_data["normalized_price"].min(),
                pos_data["normalized_price"].max(),
                100,
            )
...
def save_missing_players_report(
    drafts_df: pd.DataFrame, results_df: pd.DataFrame, output_dir: Path
):
    ...
    merged = drafts_df.merge(
        results_df, on=["season_year", "player_id"], how="left", indicator=True
    )
    ...
    if not missing.empty:
        ...
        missing_report.columns = [
            c.replace("_draft", "").replace("_result", "")
            for c in missing_report.columns
        ]

        if "cost" in missing_report.columns:
            missing_report = missing_report.sort_values(
                ["season_year", "cost"], ascending=[True, False]
            )
        else:
        ...
        logger.warning(
            f"Found {len(missing_report)} players missing from results - saved to {output_path}"
        )
    else:
        logger.info("All drafted players found in results")


def plot_faab_vs_var(waiver_pickups_df: pd.DataFrame, output_dir: Path):
    ...
    if "pickup_type" in df_plot.columns:
        ...
        ax.scatter(
            df_plot["acquisition_cost"], df_plot["var_after_pickup"], alpha=0.6, s=50
        )
...
def plot_var_by_source(manager_profiles_df: pd.DataFrame, output_dir: Path):
    ...
    if "season_year" in manager_profiles_df.columns:
        by_season = (
            manager_profiles_df.groupby("season_year")
            .agg(
                {
                    "draft_var": "sum",
                    "keeper_var": "sum",
