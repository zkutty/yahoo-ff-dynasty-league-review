...
def normalize_prices(
    drafts_df: pd.DataFrame, league_meta: Dict, baseline_season: int = 2014
) -> pd.DataFrame:
    ...
    baseline_budget = baseline_meta.get(
        "num_teams", config.NUM_TEAMS
    ) * baseline_meta.get("auction_budget", config.AUCTION_BUDGET)
    ...
    for year in df["season_year"].unique():
        ...
        if not meta:
            ...
            num_teams = config.NUM_TEAMS
            auction_budget = config.AUCTION_BUDGET
        ...
        total_starters = (
            sum(starting_slots.values()) if starting_slots else 9
        )  # QB+2RB+2WR+TE+FLEX+DEF+K
        ...
        effective_budget_per_spot = (
            remaining_budget / remaining_roster_spots
        ...
        baseline_meta = league_meta.get(baseline_season, {})
        baseline_num_teams = baseline_meta.get("num_teams", config.NUM_TEAMS)
        baseline_auction_budget = baseline_meta.get(
            "auction_budget", config.AUCTION_BUDGET
        )
        ...
        baseline_total_starters = (
            sum(baseline_starting_slots.values()) if baseline_starting_slots else 9
        )
        baseline_bench_slots = baseline_meta.get("bench_slots", 6)
        baseline_total_spots = baseline_num_teams * (
            baseline_total_starters + baseline_bench_slots
        )

        # For baseline season, check if there were keepers
        baseline_season_drafts = df[df["season_year"] == baseline_season]
        baseline_keeper_spend = baseline_season_drafts[
            baseline_season_drafts["is_keeper"] == True
        ]["cost"].sum()
        ...
        baseline_budget_per_spot = (
            baseline_remaining_budget / baseline_remaining_spots
            if baseline_remaining_spots > 0
            else baseline_auction_budget
        )

        # Inflation factor
        inflation_factor = (
            effective_budget_per_spot / baseline_budget_per_spot
            if baseline_budget_per_spot > 0
            else 1.0
        )
