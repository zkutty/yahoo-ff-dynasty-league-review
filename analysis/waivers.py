...
def analyze_waiver_pickups(
    ...
    for _, pickup in waiver_pickups.iterrows():
        ...
        cost_efficiency = (
            var_after_pickup / acquisition_cost if acquisition_cost > 0 else None
        )
        ...
        if position:
            ...
            if not position_pickups.empty and "VAR_total" in position_pickups.columns:
                ...
                if var_after_pickup is not None:
                    var_percentile = (
                        (var_values <= var_after_pickup).sum() / len(var_values) * 100
                    )

        # Classify archetype
        pickup_type = classify_pickup_archetype(
            var_after_pickup, weeks_started, weeks_rostered, position, var_percentile
        )
