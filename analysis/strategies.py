...
def build_manager_strategy_profiles(
    ...
    for (season, team_key), group in lifecycle_df.groupby(
        ["season_year", team_key_col]
    ):
        # Calculate VAR by acquisition type (use VAR_total or VAR column)
        var_col = (
            "VAR_total"
            if "VAR_total" in group.columns
            else "VAR"
        ...
        if var_col:
            draft_var = (
                group[group["acquisition_type"] == "draft"][var_col].fillna(0).sum()
            )
            keeper_var = (
                group[group["acquisition_type"] == "keeper"][var_col].fillna(0).sum()
            )
            waiver_var = (
                group[group["acquisition_type"].isin(["waiver", "free_agent"])][var_col]
                .fillna(0)
                .sum()
            )
            trade_var = (
                group[group["acquisition_type"] == "trade"][var_col].fillna(0).sum()
            )
        else:
        ...
        team_waiver_key = (
            "team_key" if "team_key" in waiver_pickups_df.columns else None
        )
        ...
        faab_spent = (
            team_waivers["acquisition_cost"].sum() if not team_waivers.empty else 0
        )
        waiver_var_actual = (
            team_waivers["var_after_pickup"].sum() if not team_waivers.empty else 0
        )
