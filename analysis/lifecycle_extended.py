...
def build_complete_lifecycle(
    ...
    if not transactions_df.empty:
        ...
        if not waiver_adds.empty:
            ...
            waiver_adds["acquisition_type"] = waiver_adds.apply(
                lambda row: "waiver"
                if (pd.notna(row.get("faab_bid")) and row.get("faab_bid", 0) > 0)
                or pd.notna(row.get("waiver_priority"))
                else "free_agent",
                axis=1,
            )
    ...
    if (
        ...
        and "waiver_adds" in locals()
        and not waiver_adds.empty
    ):
    ...
    for (season, player_id), group in acquisitions_df.groupby(
        ...
        total_points = None
        if (
            not player_results.empty
            and "fantasy_points_total" in player_results.columns
        ):
        ...
        lifecycle_list.append(
            {
                ...
                "position": position,
                "team_key": earliest.get(
                    "team_key"
                ),  # Earliest team (may change if traded)
