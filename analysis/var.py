...
def calculate_replacement_baseline(
    results_df: pd.DataFrame, league_meta: Dict, season: int
) -> Dict[str, float]:
        ...
        if position == "FLEX":
            ...
            for flex_pos in ["RB", "WR", "TE"]:
                ...
                if flex_rank <= len(pos_results):
                    flex_replacements.append(
                        pos_results.iloc[flex_rank - 1]["fantasy_points_total"]
                    )
            replacement_baselines[position] = (
                max(flex_replacements) if flex_replacements else 0.0
            )
        else:
            replacement_rank = num_teams * starters_at_pos

            if replacement_rank <= len(pos_results):
                replacement_baselines[position] = pos_results.iloc[
                    replacement_rank - 1
                ]["fantasy_points_total"]
            else:
                # If not enough players, use median or minimum
                replacement_baselines[position] = pos_results[
                    "fantasy_points_total"
                ].min()
...
def calculate_var(
    results_df: pd.DataFrame, drafts_df: pd.DataFrame, league_meta: Dict
) -> pd.DataFrame:
    ...
    merged["dollar_per_VAR"] = merged["dollar_per_VAR"].replace(
        [np.inf, -np.inf], np.nan
    )
    merged["VAR_per_dollar"] = merged["VAR_per_dollar"].replace(
        [np.inf, -np.inf], np.nan
    )
